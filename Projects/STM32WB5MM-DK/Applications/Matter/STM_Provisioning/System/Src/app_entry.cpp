...
const osThreadAttr_t ShciUserEvtProcess_attr = { .name = CFG_SHCI_USER_EVT_PROCESS_NAME, .attr_bits = CFG_SHCI_USER_EVT_PROCESS_ATTR_BITS, .cb_mem =
CFG_SHCI_USER_EVT_PROCESS_CB_MEM, .cb_size = CFG_SHCI_USER_EVT_PROCESS_CB_SIZE, .stack_mem = CFG_SHCI_USER_EVT_PROCESS_STACK_MEM, .stack_size =
		CFG_SHCI_USER_EVT_PROCESS_STACK_SIZE, .priority = CFG_SHCI_USER_EVT_PROCESS_PRIORITY };

const osThreadAttr_t PushButtonProcess_attr = { .name = CFG_PUSH_BUTTON_EVT_PROCESS_NAME, .attr_bits = CFG_PUSH_BUTTON_EVT_PROCESS_ATTR_BITS,
		.cb_mem = CFG_PUSH_BUTTON_EVT_PROCESS_CB_MEM, .cb_size = CFG_PUSH_BUTTON_EVT_PROCESS_CB_SIZE, .stack_mem =
		CFG_PUSH_BUTTON_EVT_PROCESS_STACK_MEM, .stack_size = CFG_PUSH_BUTTON_EVT_PROCESS_STACK_SIZE, .priority =
		CFG_PUSH_BUTTON_EVT_PROCESS_PRIORITY };

#if (ENABLE_IWDG_SUPPORT == 1)
const osThreadAttr_t IWDSBSFU_attr = { .name = CFG_IWD_SBSFU_EVT_PROCESS_NAME, .attr_bits =
        CFG_IWD_SBSFU_EVT_PROCESS_ATTR_BITS, .cb_mem =
		CFG_IWD_SBSFU_EVT_PROCESS_CB_MEM, .cb_size = CFG_IWD_SBSFU_EVT_PROCESS_CB_SIZE, .stack_mem =
		CFG_IWD_SBSFU_EVT_PROCESS_STACK_MEM, .stack_size = CFG_IWD_SBSFU_EVT_PROCESS_STACK_SIZE, .priority =
		CFG_IWD_SBSFU_EVT_PROCESS_PRIORITY };
#endif /* (ENABLE_IWDG_SUPPORT == 1) */
...
void APPE_Init(void) 
{
	 ...
	 * Once running, IWDG cannot be stopped. UserApp must refresh it periodically before it causes a reset.
	 * We re-use IWDG to monitor application crashes, but we need to increase IWDG counter value.
	 * As IWDG registers are protected, it is required first to write a specific value in the KR register.
     *   LSI_freq = 32000 Hz
	 *   IWDG prescaler (IWDG_PR) is applied to LSI.
	 *   Reload register (IWDG_RLR) is a 12-bit register (maximum value = 0xFFF).
	 *   IWDG timeout = IWDG_RSR * IWDG_PR / 32000
	 * Settings for a 25 sec timeout :
	*/
    WRITE_REG(IWDG->KR, IWDG_KEY_WRITE_ACCESS_ENABLE); /* enable modification of IWDG registers */
    WRITE_REG(IWDG->PR, IWDG_PRESCALER_256); 
    WRITE_REG(IWDG->RLR, 0xC35);
	/* refresh the IWDG */
...
static void APPE_SysEvtReadyProcessing(void) {
    ...
    APP_DBG("Initialisation finish");

    error = FACTORYDATA_Write();
    uint8_t msg[UART_MSG_SIZE];
    snprintf((char *)msg,sizeof(msg),"finish:%03u",error);
    while(1){

    	HAL_StatusTypeDef status = HAL_UART_Transmit(&huart1, msg, sizeof(msg), 0xFFFF);
    	if(status != HAL_OK)
    	{
    		Error_Handler();
    	}
...
static void IWDSBSFUEvtProcess(void *argument) {
    ...
    for (;;) {
        ...
        vTaskDelayUntil(&xLastWakeTime, IWD_RELOAD);
