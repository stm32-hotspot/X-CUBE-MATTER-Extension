...
CHIP_ERROR AppTask::Init() {
    ...
    if (chip::Server::GetInstance().GetFabricTable().FabricCount() == 0) {
		...
		UTIL_LCD_DisplayStringAt(70, 12, (uint8_t*) "Thermo-", LEFT_MODE);
		UTIL_LCD_DisplayStringAt(70, 24, (uint8_t*) "stat App", LEFT_MODE);
		BSP_LCD_Refresh(0);
...
void AppTask::UpdateTempGUI(void) {
    ...
    BSP_LCD_Refresh(0);
#endif
#if (CFG_RGB_LED_SUPPORTED == 1)
    LL_GPIO_SetPinMode(GPIOA, LL_GPIO_PIN_7, LL_GPIO_MODE_OUTPUT);

    if (mode == ThermMode::SystemModeEnum::kOff) UpdateRgbLed(0,0,0);
    else if ((mode == ThermMode::SystemModeEnum::kCool || mode == ThermMode::SystemModeEnum::kAuto) && tempi > cool_set) UpdateRgbLed(0,0,10);
    else if ((mode== ThermMode::SystemModeEnum::kHeat || mode == ThermMode::SystemModeEnum::kAuto) && tempi < heat_set) UpdateRgbLed(10,0,0);
    else UpdateRgbLed(0,10,0);
#endif
}
...
void AppTask::ButtonRelease(AppEvent *aEvent) {
    ...
    if (sAppTask.mFunction == kFunction_ShortRelease) {
        APP_DBG("BUTTON 2 OnShortRelease");
        ...
        APP_DBG("BUTTON 2 OnLongRelease")
        if (mode == ThermMode::SystemModeEnum::kAuto) {
            TempMgr().SetSetpoints(-100, -100);
        } else if (mode == ThermMode::SystemModeEnum::kCool) {
            TempMgr().SetSetpoints(-100, 0);
    ...
    } else if (sAppTask.mFunction == kFunction_LongPress) APP_DBG("BUTTON 2 OnLongPress");
}
