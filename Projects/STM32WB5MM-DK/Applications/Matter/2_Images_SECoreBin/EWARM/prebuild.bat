...
set "command=%python%%prepareimage% trans -r True -k %oemkey% -f SE_ReadKey_1 -v %cortex% >> %asmfile%"
%command%
IF %ERRORLEVEL% NEQ 0 goto error

set "oemkey=%1\\..\\Binary\\OEM_KEY_COMPANY2_key_AES_%type%.bin"
IF NOT EXIST %oemkey% goto AES128_end
set "command=%python%%prepareimage% trans -r True -k %oemkey% -f SE_ReadKey_2 -v %cortex% >> %asmfile%"
%command%
IF %ERRORLEVEL% NEQ 0 goto error

set "oemkey=%1\\..\\Binary\\OEM_KEY_COMPANY3_key_AES_%type%.bin"
IF NOT EXIST %oemkey% goto AES128_end
set "command=%python%%prepareimage% trans -r True -k %oemkey% -f SE_ReadKey_3 -v %cortex% >> %asmfile%"
%command%
IF %ERRORLEVEL% NEQ 0 goto error
