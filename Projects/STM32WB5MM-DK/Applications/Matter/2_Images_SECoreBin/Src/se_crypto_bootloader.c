...
static void SE_CRYPTO_AES_ReadKey(SE_FwRawHeaderTypeDef *pxSE_Metadata)
{
  ...
  if (memcmp(pxSE_Metadata->SFUMagic, SFUM_1, strlen(SFUM_1)) == 0)
  {
    ...
    key = 0U;
#else /*CKS_ENABLED*/
    /*  FIXME: We modified the python script to generate the key code 
        that can be loaded to AES key registers directly. 
        No temp RAM buffer required nor key data swapping required 
        key data inverse and key buffer in RAM clearing is not needed
    */
    SE_ReadKey_1((uint8_t*)&hcryp1.Instance->KEYR0);
#endif   /*CKS_ENABLED*/ 
  }
  ...
  else if (memcmp(pxSE_Metadata->SFUMagic, SFUM_2, strlen(SFUM_2)) == 0)
  {
    ...
    key = 1U;
#else /*CKS_ENABLED*/
    SE_ReadKey_2((uint8_t*)&hcryp1.Instance->KEYR0);
#endif   /*CKS_ENABLED*/ 
  }
  ...
  else if (memcmp(pxSE_Metadata->SFUMagic, SFUM_3, strlen(SFUM_3)) == 0)
  {
    ...
    key = 2U;
#else /*CKS_ENABLED*/
    SE_ReadKey_3((uint8_t*)&hcryp1.Instance->KEYR0);
#endif   /*CKS_ENABLED*/ 
  }
