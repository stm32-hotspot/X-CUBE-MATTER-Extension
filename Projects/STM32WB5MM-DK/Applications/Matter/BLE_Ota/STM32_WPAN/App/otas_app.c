...
#ifdef OTA_SBSFU
          OTAS_APP_Context.base_address = SLOT_DWL_4_START;
          if( SLOT_DWL_4_START  < EXTERNAL_FLASH_ADDRESS)
          {
            first_sector = (SLOT_DWL_4_START&0x00FFF000)>>12;
            nb_sectors = (((SLOT_DWL_4_END-SLOT_DWL_4_START)&0x00FFF000)>>12)+1;
            FD_EraseSectors(first_sector, nb_sectors);
          }
          else
          {
            EXT_FD_EraseSectors(SLOT_DWL_4_START, (SLOT_DWL_4_END-SLOT_DWL_4_START));
          }
#endif /* OTA_SBSFU */
          break;
