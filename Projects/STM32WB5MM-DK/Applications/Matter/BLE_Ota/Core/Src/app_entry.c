...
#define PWM_LED_HANDLING_DELAY 5
#define PWM_LED_BLINKING_DELAY 500
...
static void Led_Init( void )
{
...
#ifdef STM32WB5MM_DK_BSP_VERSION
  BSP_PWM_LED_Init();
  BSP_PWM_LED_Off();
  HAL_Delay(PWM_LED_HANDLING_DELAY);

  /* Red LED on to indicate initialization is ongoing */
  ...
  HAL_Delay(PWM_LED_BLINKING_DELAY);
#else /* STM32WB5MM_DK_BSP_VERSION */
...
static SHCI_TL_UserEventFlowStatus_t APPE_SysevtReadyProcessing( SHCI_C2_Ready_Evt_t *pReadyEvt )
{
  ...
  BSP_PWM_LED_Off();
  HAL_Delay(PWM_LED_HANDLING_DELAY);
  /* Green LED on to indicate processing */
