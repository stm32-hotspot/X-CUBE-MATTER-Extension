                                         ...
                                         SE_FwRawHeaderTypeDef *pTestHeader)
{
  ...
  SE_FwRawHeaderTypeDef *p_backup_header;
  ...
  /*
   * Save new installed FW in backup slot
   */
  /* Read backup header */
  // FIXME: Add trace print to know whether the downloaded FW is copied to backup slot
  TRACE("\r\n\t[InstallNewVersion] Check FW in backup slot\r\n");
  p_buffer = (uint8_t *) SlotHeaderAdd[SLOT_BACKUP];
  if (SFU_LL_FLASH_Read(buffer, p_buffer, sizeof(buffer)) == SFU_SUCCESS)
  {
    p_backup_header = (SE_FwRawHeaderTypeDef *)(uint32_t)buffer;

    /* Verify if this new installed FW is already savec in backup slot */
    if (MemoryCompare(p_backup_header->FwTag, pTestHeader->FwTag, SE_TAG_LEN) != SFU_SUCCESS)
    {
      // FIXME: Add trace print to know whether the downloaded FW is copied to backup slot
      TRACE("\r\n\t[InstallNewVersion] Copy from download slot to backup slot\r\n");  
      /* Save new installed FW in backup slot */
      e_ret_status = CopyFw(DwlSlot, SLOT_BACKUP, pTestHeader->FwSize + SFU_IMG_IMAGE_OFFSET);
      if (e_ret_status != SFU_SUCCESS)
      {
        SFU_EXCPT_SetError(SFU_EXCPT_BACKUP_COPY_FAILURE_ERR);
        return e_ret_status;
      }
    }
    // FIXME: Add trace print to know whether the downloaded FW is copied to backup slot
    ...
    {
      TRACE("\r\n\t[InstallNewVersion] FW already in backup slot\r\n");
    }
  }
  else
  {
    TRACE("\r\n\t[InstallNewVersion] Read header from backup slot failed\r\n");
  }
...
/**
  * @brief  Check that there is a backup Image to Install
  *         Active slot is empty
  *         Active and backup header are equal
  * @param  ActiveSlot identification of the active area
  * @param  BackupSlot identification of the backup area
  * @retval SFU_SUCCESS if Image can be installed, a SFU_ERROR  otherwise.
  */
static SFU_ErrorStatus BackupToInstall(uint32_t ActiveSlot, uint32_t BackupSlot, SE_FwRawHeaderTypeDef *pFwImageHeader)
{
  SFU_ErrorStatus e_ret_status = SFU_ERROR;
  uint8_t *p_buffer;
  uint8_t  buffer[SE_FW_HEADER_TOT_LEN];
  SE_FwRawHeaderTypeDef *p_active_header;

  /* Read header */
  p_buffer = (uint8_t *) SlotHeaderAdd[ActiveSlot];
  if (SFU_LL_FLASH_Read(buffer, p_buffer, sizeof(buffer)) == SFU_SUCCESS)
  {
    /* Check if the FW header is authentic : SFU1 / SFU2 / SFU3 */
    p_active_header = (SE_FwRawHeaderTypeDef *)(uint32_t)buffer;
    if (SFU_IMG_GetFwImageSlot(p_active_header) != SLOT_INACTIVE)
    {
      /*
       ...
       */
      if (VerifySlot((uint8_t *) SlotStartAdd[ActiveSlot], SFU_IMG_IMAGE_OFFSET + 0x20U, 0U) == SFU_SUCCESS)
      {
        /* Verify FW Image Header tag in backup slot */
        if (SFU_SUCCESS == CheckAndGetFWHeader(BackupSlot, pFwImageHeader))
        {
          /* same signature ? */
          e_ret_status = MemoryCompare(p_active_header->FwTag, pFwImageHeader->FwTag, SE_TAG_LEN);

          /* Erase back-up if not equal */
          if (e_ret_status == SFU_ERROR)
          {
#if defined(SFU_VERBOSE_DEBUG_MODE)
            TRACE("\r\n\t  Backup and active headers are not equal : erasing ...");
#endif /* SFU_VERBOSE_DEBUG_MODE */
            (void) SFU_IMG_EraseDownloadedImg(BackupSlot);
          }
        }
      }
    }
                                                                ...
                                                                uint32_t *pActiveSlotToRollback)
{
  ...
  *pDwlSlotToInstall = SLOT_INACTIVE;
  ...
  /* Back-up FW available ?
     ====================== */
  if (SFU_SUCCESS == BackupToInstall(SLOT_ACTIVE_1, SLOT_BACKUP, &fw_image_header_to_test))
  {
    /*
     * A Backup firmware is available for installation:
     * copy SLOT_BACKUP in SLOT_DWL_1
     * fw_image_header_to_test has been populated
     */
    if (SFU_SUCCESS == CopyFw(SLOT_BACKUP, SLOT_DWL_1, fw_image_header_to_test.FwSize + SFU_IMG_IMAGE_OFFSET))
    {
      *pDwlSlotToInstall = SLOT_DWL_1;
...
SFU_ErrorStatus SFU_IMG_TriggerResumeInstallation(uint32_t ActiveSlot, uint32_t DwlSlot)
{
  ...
  // FIXME >>>>: Add copy of new version from download slot to backup slot
  // Without this fix, in case of FW install resume, new version from download slot 1 will not be copied to backup slot 
  /*
   * Save new installed FW in backup slot
   */
  /* Read backup header */
  {
    ...
    uint8_t  buffer[SE_FW_HEADER_TOT_LEN];
    SE_FwRawHeaderTypeDef *p_backup_header;
    SE_FwRawHeaderTypeDef *pTestHeader = &fw_image_header_to_test;
    ...
    if (SFU_LL_FLASH_Read(buffer, p_buffer, sizeof(buffer)) == SFU_SUCCESS)
    {
      ...
      /* Verify if this new installed FW is already saved in backup slot */
      if (MemoryCompare(p_backup_header->FwTag, pTestHeader->FwTag, SE_TAG_LEN) != SFU_SUCCESS)
      {
        TRACE("\r\n\t[InstallNewVersion] Copy from download slot to backup slot\r\n");  
        /* Save new installed FW in backup slot */
