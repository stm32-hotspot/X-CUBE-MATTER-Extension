...
#include "sfu_error.h"
#include "stm32wbxx_it.h" /* required for the HAL Cube callbacks */
/*
...
static void SFU_BOOT_SM_CheckNewFwToDownload(void)
{
...
#if (SECBOOT_LOADER == SECBOOT_USE_STANDALONE_LOADER)
  SFU_ErrorStatus  e_flash_ret_status = SFU_ERROR;
  SFU_FLASH_StatusTypeDef flash_if_status;
  SE_StatusTypeDef e_se_status = SE_KO;

  /* Bypass mode ==> Copy the CM0 binary from the external flash into the internal flash */
  if ((STANDALONE_LOADER_STATE == STANDALONE_LOADER_BYPASS_REQ) ||
      (STANDALONE_LOADER_STATE == STANDALONE_LOADER_BYPASS_REQ_AND_INSTALL_REQ))
  {
    
      StackSize = SLOT_SIZE(SLOT_DWL_4);
    if (StackSize > 0)
    {
      if (CopyFw_Stack(SLOT_DWL_4, SLOT_ACTIVE_1, StackSize) == SFU_SUCCESS)
      {
        TRACE("\r\n\t  CM0 binary successfully copied");

        //FIXME: >>> Computime point 1: Keep the M0+ stack on external flash 
        // FIXME: we keep the stack in download slot 4 to allow app to trigger another update next time
        /*  e_flash_ret_status = SFU_LL_FLASH_Erase_Size(&flash_if_status, (uint8_t *) SlotStartAdd[SLOT_DWL_4],
                                                     SLOT_SIZE(SLOT_DWL_4));
        STATUS_FWIMG(e_flash_ret_status == SFU_ERROR, SFU_IMG_FLASH_ERASE_FAILED);
        if(e_flash_ret_status != SFU_SUCCESS)
        {
          SFU_EXCPT_SetError(SFU_EXCPT_CM0_UPDATE_ERR);
  ...
  /* Bypass mode ==> CM0 update process to be started / in progress */
  if ((STANDALONE_LOADER_STATE == STANDALONE_LOADER_BYPASS) ||
      (STANDALONE_LOADER_STATE == STANDALONE_LOADER_BYPASS_REQ) ||
      (STANDALONE_LOADER_STATE == STANDALONE_LOADER_BYPASS_AND_INSTALL_REQ) ||
      (STANDALONE_LOADER_STATE == STANDALONE_LOADER_BYPASS_REQ_AND_INSTALL_REQ))
  {
    TRACE("\r\n= [SBOOT] STATE: Bypass mode - execution CM0 update process");
    if (SE_CM0_Update(&e_se_status) == SE_SUCCESS)
    {
      TRACE("\r\n\t  CM0 update successful");
      e_ret_status = SFU_SUCCESS;
    }
    else
    {
      e_flash_ret_status = SFU_LL_FLASH_Erase_Size(&flash_if_status, (uint8_t *) SlotStartAdd[SLOT_ACTIVE_1],
                                                   SLOT_SIZE(SLOT_ACTIVE_1));
      STATUS_FWIMG(e_flash_ret_status == SFU_ERROR, SFU_IMG_FLASH_ERASE_FAILED);
      SFU_EXCPT_SetError(SFU_EXCPT_CM0_UPDATE_ERR);
      STANDALONE_LOADER_STATE = STANDALONE_LOADER_NO_REQ;
      e_ret_status = SFU_ERROR;
