    ...
    if (fw_verified_total_size <= fw_size)
    {
      ...
      if ((se_ret_status == SE_SUCCESS) && (*pSeStatus == SE_OK) && (fw_tag_len == SE_TAG_LEN))
      {
        ...
        else
        {
          ...
          /* Save result for active slot :
             to avoid single fault attach the signature will be double checked before UserApp execution */
          if ((SlotNumber >= SLOT_ACTIVE_1) && (SlotNumber < SLOT_DWL_1))
          {
...
uint32_t GetStackSize(uint32_t SlotOrigine, uint32_t MximumOffset)
{
  SFU_ErrorStatus e_ret_status = SFU_ERROR;
  
  uint8_t nbsector = 0;
  uint32_t wireless_stack_size = 0;
  uint64_t read64 = 0;
  uint32_t flash_current_offset = 0;
  bool binary_parsing_on_going = true;
  bool error_keyword_or_size = false;
  bool magic_key_low32bit = false;
  
  while(binary_parsing_on_going == true){
    
    e_ret_status = SFU_LL_FLASH_Read( (uint8_t *)&read64, (uint8_t *)(SlotStartAdd[SlotOrigine] + flash_current_offset), 8);
    if (e_ret_status == SFU_SUCCESS)
    {
      /* Test if current word contains magic keyword */
      if((read64 & 0x00000000FFFFFFFF) == FUOTA_MAGIC_KEYWORD_COPRO_WIRELESS)
      {
        binary_parsing_on_going = false;
        magic_key_low32bit = true;
      }
      else if (((read64 & 0xFFFFFFFF00000000) >> 32) == FUOTA_MAGIC_KEYWORD_COPRO_WIRELESS)
      {
        binary_parsing_on_going = false;
      }
      ...
      if(flash_current_offset > MximumOffset)
      {
        error_keyword_or_size = true;
        binary_parsing_on_going = false;
      }
    }
    else //failed to read external flash
    {
       error_keyword_or_size = true;
      break;
    }
  ...
  {
    TRACE("Error:  WIRELESS Stack Keyword not found in FLASH memory, aborting!");
  }
  else
  { 
    if (magic_key_low32bit)
    {
      e_ret_status = SFU_LL_FLASH_Read( &nbsector, (uint8_t *)(SlotStartAdd[SlotOrigine] + flash_current_offset - 16), 1);
    }
    else
    {
      e_ret_status = SFU_LL_FLASH_Read( &nbsector, (uint8_t *)(SlotStartAdd[SlotOrigine] + flash_current_offset - 12), 1);    
    }
    
    TRACE("\r\n flash_current_offset is %d\r\n", flash_current_offset);
    TRACE("\r\n nbsector 0x%02x\r\n", nbsector);
    
    if (e_ret_status == SFU_SUCCESS)
    {
      wireless_stack_size = nbsector << 12;
    }
    
    TRACE("\r\n Stack size is 0x%08x (aligned to 4KB)\r\n", wireless_stack_size);
  }
  
  return wireless_stack_size;
}

/**
  * @brief  Copy Wireless Stack from slot Origine to slot Destination
  * @param  SlotOrigine Origine of the copy
  * @param  SlotDestination Destination of the copy
  * @param  Length Stack length to copy
  * @retval SFU_ErrorStatus SFU_SUCCESS if successful, SFU_ERROR otherwise.
  */
SFU_ErrorStatus CopyFw_Stack(uint32_t SlotOrigine, uint32_t SlotDestination, uint32_t Length)
{
  SFU_ErrorStatus e_ret_status = SFU_ERROR;
  ...
  uint32_t OldStackAddr;
  uint32_t NewStackAddr;


//  /* Destination slot size should be at least >= length to copy */
//  if (SLOT_SIZE(SlotDestination) <  Length)
//  {
//    return SFU_ERROR;
//  }

  /* Origin slot size should be at least >= length to copy */
  if (SLOT_SIZE(SlotOrigine) <  Length)
  {
    return SFU_ERROR;
  }
  ...
  OldStackAddr = 0x08000000 | (LL_FLASH_GetSFSA() << 12);
  TRACE("OldStackAddr: 0x%08X \n\r", OldStackAddr);
  
  NewStackAddr = OldStackAddr - Length - 4096;
  TRACE("NewStackAddr: 0x%08X \n\r", NewStackAddr);
  
  /* Free area size should be at least >= length to copy */
  if ((OldStackAddr - SlotStartAdd[SlotDestination] - 4096) < Length)
  {
    return SFU_ERROR;
  }
  
  TRACE("\r\n= [CopyFw] Copy from @%x to @%x\r\n", SlotStartAdd[SlotOrigine], NewStackAddr);
  
/* Erase destination slot
     ====================== */
  e_ret_status = SFU_LL_FLASH_Erase_Size(&flash_if_status, (uint8_t *) SlotStartAdd[SlotDestination],
                                         OldStackAddr - SlotStartAdd[SlotDestination]);
  STATUS_FWIMG(e_ret_status == SFU_ERROR, SFU_IMG_FLASH_ERASE_FAILED);

  /* Copy from origine to destination
     ================================ */
  while ((e_ret_status == SFU_SUCCESS) && (index < Length))
  {
    ...
    /* Read the write if no error */
    e_ret_status = SFU_LL_FLASH_Read(buffer, (uint8_t *)(SlotStartAdd[SlotOrigine]+ index), size);
    STATUS_FWIMG(e_ret_status == SFU_ERROR, SFU_IMG_FLASH_READ_FAILED);
    if (e_ret_status == SFU_SUCCESS)
    {
      e_ret_status = SFU_LL_FLASH_Write(&flash_if_status, (uint8_t *)(NewStackAddr + index), buffer, size);
...
/**
  * @brief  Copy from slot Origine to slot Destination
  * @param  SlotOrigine Origine of the copy
  * @param  SlotDestination Destination of the copy
  * @param  Length Length to copy
  * @retval SFU_ErrorStatus SFU_SUCCESS if successful, SFU_ERROR otherwise.
  */
SFU_ErrorStatus CopyFw(uint32_t SlotOrigine, uint32_t SlotDestination, uint32_t Length)
{
  SFU_ErrorStatus e_ret_status = SFU_ERROR;
  uint8_t buffer[SFU_IMG_CHUNK_SIZE] __attribute__((aligned(8)));
  SFU_FLASH_StatusTypeDef flash_if_status;
  ...
  //FIXME : Add debug print for FW copy
  TRACE("\r\n= [CopyFw] Copy from @%x to @%x\r\n", SlotStartAdd[SlotOrigine], SlotStartAdd[SlotDestination]);
  /* Destination slot size should be at least >= length to copy */
  if (SLOT_SIZE(SlotDestination) <  Length)
  {
    return SFU_ERROR;
  ...
  while ((e_ret_status == SFU_SUCCESS) && (index < Length))
  {
    ...
    if (e_ret_status == SFU_SUCCESS)
    {
      e_ret_status = SFU_LL_FLASH_Write(&flash_if_status, (uint8_t *)(SlotStartAdd[SlotDestination] + index), buffer, size);
      STATUS_FWIMG(e_ret_status == SFU_ERROR, SFU_IMG_FLASH_WRITE_FAILED);
...
/**
  * @brief  Check whether there is a backup image to restore
  *         in the download slot
  * @param  DwlSlot identification of the download area
  * @param  BackupSlot identification of the backup area
  * @retval the FW version if the download and backup headers
  *         are valid and equal (coded on uint16_t), 0 otherwise
  */
uint16_t SFU_IMG_CheckBackupToRestore(uint32_t DwlSlot, uint32_t BackupSlot)
{
  SE_FwRawHeaderTypeDef dwl_fw_image_header;
  SE_FwRawHeaderTypeDef backup_fw_image_header;
  SFU_ErrorStatus e_ret_status = SFU_ERROR;
  uint16_t version = 0;

  /* Check the header of the download slot */
  e_ret_status = CheckAndGetFWHeader(DwlSlot, &dwl_fw_image_header);
  if (e_ret_status == SFU_SUCCESS)
  {
    /*
     ...
     */
    if (VerifySlot((uint8_t *) SlotStartAdd[DwlSlot], SFU_IMG_IMAGE_OFFSET + 0x20U, 0U) == SFU_SUCCESS)
    {
      /* Check the header of the backup slot */
      e_ret_status = CheckAndGetFWHeader(BackupSlot, &backup_fw_image_header);
      if (e_ret_status == SFU_SUCCESS)
      {
        /* Are the signatures identical? */
        e_ret_status = MemoryCompare((uint8_t *)&dwl_fw_image_header.FwTag, (uint8_t *)&backup_fw_image_header.FwTag, SE_TAG_LEN);
        if (e_ret_status == SFU_SUCCESS)
        {
          /* Retrieve the version from the header without any further check */
          version = dwl_fw_image_header.FwVersion;
        }
      }
    }
  }
...
SFU_ErrorStatus SFU_IMG_CheckCandidateVersion(uint32_t DwlSlot)
{
  ...
  /* The active slot is invalid
   * Check whether there is a backup image to restore. */
  if (cur_ver == 0U)
  {
    cur_ver = SFU_IMG_CheckBackupToRestore(DwlSlot, SLOT_BACKUP);
  }
  ...
  else
  {
    ...
    if (verif_ver == 0U)
    {
      verif_ver = SFU_IMG_CheckBackupToRestore(DwlSlot, SLOT_BACKUP);
    }
