...
Depending on the wireless stack integrated, the slot size allocated to the UserApp can be extended accordingly to the
Secure Flash Start Address option byte (SFSA).
In this example, only the first 512 Kbytes are allocated to the non secure flash.
For example, when integrating stm32wb5x_BLE_Stack_light_fw.bin, SFSA is set to 0xD5 (0x080D5000).
...
@par Directory contents
   ...
   - 2_Images_SBSFU/Core/Src/stm32wbxx_hal_msp.c              HAL MSP module
   - 2_Images_SBSFU/Core/Src/stm32wbxx_it.c                   STM32 interrupt handlers
   ...
   - 2_Images_SBSFU/Core/Inc/stm32wbxx_hal_conf.h             HAL configuration file
   - 2_Images_SBSFU/Core/Inc/stm32wbxx_it.h                   Header file for stm32wbxx_it.c
...
8. Send the user encrypted firmware file (\2_Images\2_Images_UserApp\Binary\UserApp.sfb) with STM32CubeMonitor-RF following
   steps provided in BLE_Ota\readme.txt

Note1 : Press User-B1 at reset to force a local download if an application is already installed.
