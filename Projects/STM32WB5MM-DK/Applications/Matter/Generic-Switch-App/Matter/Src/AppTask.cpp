...
#define STM32_LIGHT_ENDPOINT_ID 2
...
static bool sSendPB2LongPress = false;
...
namespace {
constexpr chip::EndpointId kGenericSwitchEndpoint = 1;
} // namespace
...
CHIP_ERROR AppTask::Init() {
    ...
    sPushButtonTimeoutTimer = xTimerCreate("PushButtonTimer", // Just a text name, not used by the RTOS kernel
            ...
            TimerNvmEventHandler // timer callback handler
            );
    ...
    sPushButton2TimeoutTimer = xTimerCreate("PushCommandButtonTimer", // Just a text name, not used by the RTOS kernel
            ...
            true,               // no timer reload (==one-shot)
            (void*) this,       // init timer id
            TimerPB2EventHandler // timer callback handler
            );
    ...
    err = GenericSwitchManager::GetInstance().Init(kGenericSwitchEndpoint);	
    if (err != CHIP_NO_ERROR) {
        APP_DBG("LightSwitchManager Init failed!");
    }
...
void AppTask::TimerNvmEventHandler(TimerHandle_t xTimer) {
...
void AppTask::TimerPB2EventHandler(TimerHandle_t xTimer) {

    B2TimerCpt++;
    if (BSP_PB_GetState(APP_FUNCTION_BUTTON2) == 0) {
        if (++B2ButtonStateCpt >= 3) {
			if (sSendPB2LongPress == false) {
				AppEvent event;
				event.Type = AppEvent::kEventType_Timer;
                event.Handler = UpdateGenericSwitchEventHandler;				
				sAppTask.mFunction = kFunction_LongPress;
				sAppTask.PostEvent(&event);
				sSendPB2LongPress = true;
			}
        }
    } else if (sSendPB2LongPress == true) {
        ...
        sAppTask.PostEvent(&event);
    } else if ((B2TimerCpt > B2ButtonStateCpt) && (B2TimerCpt < 3)) {
        xTimerStop(sPushButton2TimeoutTimer, 0);
        AppEvent event;
        event.Type = AppEvent::kEventType_Timer;
        event.Handler = UpdateGenericSwitchEventHandler;
...
void AppTask::FunctionHandler(AppEvent *aEvent) {
    if (aEvent->Type == AppEvent::kEventType_Button) {
        if (aEvent->ButtonEvent.ButtonIdx == APP_FUNCTION_BUTTON1) {
            if (xTimerIsTimerActive(sPushButtonTimeoutTimer) == 0) {
                xTimerStart(sPushButtonTimeoutTimer, 0);
                NvmTimerCpt = 0;
                NvmButtonStateCpt = 0;
            }
        } else if (aEvent->ButtonEvent.ButtonIdx == APP_FUNCTION_BUTTON2) {
            if (sHaveFabric == true) { // only use if a fabric is available
                if (xTimerIsTimerActive(sPushButton2TimeoutTimer) == 0) {
                    xTimerStart(sPushButton2TimeoutTimer, 0);
                    B2TimerCpt = 0;
                    B2ButtonStateCpt = 0;
                    sSendPB2LongPress = false;
                    APP_DBG("BUTTON 2 GenericSwitchOnInitialPress");
                    GenericSwitchManager::GetInstance().GenericSwitchOnInitialPress();
                }
            }
        }
    }
...
void AppTask::UpdateGenericSwitchEventHandler(AppEvent *aEvent) {

    if (sAppTask.mFunction == kFunction_ShortRelease) {
        APP_DBG("BUTTON 2 GenericSwitchOnShortRelease");
        GenericSwitchManager::GetInstance().GenericSwitchOnShortRelease();
    } else if (sAppTask.mFunction == kFunction_LongRelease) {
        APP_DBG("BUTTON 2 GenericSwitchOnLongRelease");
        GenericSwitchManager::GetInstance().GenericSwitchOnLongRelease();
    } else if (sAppTask.mFunction == kFunction_LongPress) {
        APP_DBG("BUTTON 2 GenericSwitchOnLongPress");
        GenericSwitchManager::GetInstance().GenericSwitchOnLongPress();
    }
}
