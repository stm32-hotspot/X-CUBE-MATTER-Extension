...
#define STM32_PLUG_ENDPOINT_ID 1
...
CHIP_ERROR AppTask::Init() {
    ...
    err = SmartPlugMgr().Init();
    if (err != CHIP_NO_ERROR) {
        APP_DBG("SmartPlugMgr().Init() failed");
        return err;
    }
    SmartPlugMgr().SetCallbacks(ActionInitiated, ActionCompleted);
    ...
    if (chip::Server::GetInstance().GetFabricTable().FabricCount() == 0) {
        ...
        UTIL_LCD_DisplayStringAt(70, 12, (uint8_t*) "Smart", LEFT_MODE);
        UTIL_LCD_DisplayStringAt(70, 24, (uint8_t*) "Plug App", LEFT_MODE);
        BSP_LCD_Refresh(0);
...
void AppTask::SmartPlugActionEventHandler(AppEvent *aEvent) {
	SmartPlugManager::Action_t action;

    if (aEvent->Type == AppEvent::kEventType_Button) {
        // Toggle
        if (SmartPlugMgr().IsTurnedOn()) {
            action = SmartPlugManager::OFF_ACTION;
        } else {
            action = SmartPlugManager::ON_ACTION;
        }

        sAppTask.mSyncClusterToButtonAction = true;
        SmartPlugMgr().InitiateAction(action, 0, 0, 0);
    }
}
...
void AppTask::ActionInitiated(SmartPlugManager::Action_t aAction) {
    // Placeholder for plug action
    if (aAction == SmartPlugManager::ON_ACTION) {
        APP_DBG("plug goes on");

#ifdef USE_STM32WB5M_DK
#if (CFG_LCD_SUPPORTED == 1)
        UTIL_LCD_ClearStringLine(2);
        char Message[11];
        snprintf(Message, sizeof(Message), "LED ON");
...
void AppTask::ActionCompleted(SmartPlugManager::Action_t aAction) {
    // Placeholder for Plug action completed
    if (aAction == SmartPlugManager::ON_ACTION) {
        APP_DBG("Plug action on completed");
    } else if (aAction == SmartPlugManager::OFF_ACTION) {
        APP_DBG("Plug action off completed");
    }
...
void AppTask::UpdateClusterState(void) {
    ...
    status = Clusters::OnOff::Attributes::OnOff::Set(
             STM32_PLUG_ENDPOINT_ID, SmartPlugMgr().IsTurnedOn());
