...
//
//    if (stackLockState == DlLockState::kLocked) mState = kState_LockingCompleted;
//    else mState = kState_UnlockingCompleted;

    // chip::app::Clusters::DoorLock::Attributes::FeatureMap::Set(1, 0x0);
	chip::app::Clusters::DoorLock::Attributes::LockState::Set(1, DlLockState::kLocked);
...
{
    volatile bool action_initiated = false;
    State_t new_state;

    if (this->LOCK_ACTION == aAction)
    {
    	ChipLogProgress(NotSpecified, "BoltLockManager => LOCK ACTION");
    }
    else
    {
    	ChipLogProgress(NotSpecified, "BoltLockManager => UNLOCK ACTION");
    }
