...
CHIP_ERROR AppTask::Init() {
    ...
    TempSensMgr().SetCallbacks(ActionCompleted);
    ...
    err = AclStorageDelegate::GetInstance().Init(SaveNvm);
    if (err != CHIP_NO_ERROR) {
		APP_DBG("AclStorageDelegate().Init() failed");
		return err;
	}
    ...
    if (chip::Server::GetInstance().GetFabricTable().FabricCount() == 0) {
		...
		UTIL_LCD_DisplayStringAt(70, 12, (uint8_t*) "Tempera-", LEFT_MODE);
		UTIL_LCD_DisplayStringAt(70, 24, (uint8_t*) "ture App", LEFT_MODE);
		BSP_LCD_Refresh(0);
...
void AppTask::ActionCompleted(TempSensorManager::Action_t aAction) {
    if (aAction == TempSensorManager::LOCAL_T_ACTION) {
        APP_DBG("Temperature set completed");
    }

#if (CFG_LCD_SUPPORTED == 1U)
    char Message[18];
    int8_t temp;

    temp = TempSensMgr().GetCurrentTemp();

    UTIL_LCD_ClearStringLine(2);
    snprintf(Message, sizeof(Message), "T: %d", temp);
