...
static  uint8_t its_erase_counter = 0U;
...
psa_status_t storage_get_info(uint64_t obj_uid, void *p_obj_info, uint32_t obj_info_size)
{
  ...
  /* Clean storage prior to starting objects creation */
  if (!its_erase_counter)
  {
    if (FLASH_If_Erase_Size(p_storage, ITS_MAX_SIZE) != HAL_OK)
    {
      return PSA_ERROR_STORAGE_FAILURE;
    }
    its_erase_counter += 1U;
  }
...
/**
 * @brief Store the Device Attestation private key encrypted by HUK
 * in Flash with ITS
 * @param  DA_Private_Key : pointer to key array.
 * @param  DA_Private_Key_Size : size of the key in bytes.
 * @retval PSA Status
 */
psa_status_t storage_DA_private_Key(const uint8_t* DA_Private_Key,uint32_t DA_Private_Key_Size)
{
	...
	/* Clean storage prior to starting objects creation         */
	/* Note : if more than 1 key, the erase need to be modified */
	/* in order to erase only wanted key object                 */
	/* storage_remove might be modified to do so                */
	if (FLASH_If_Erase_Size(p_storage, ITS_MAX_SIZE) != HAL_OK)
	{
	   return PSA_ERROR_STORAGE_FAILURE;
	}

	/* Store in NVM thru ITS the DA Private key encrypted with HUK*/
	retval = psa_its_set(DEVICE_ATTESTATION_PRIVATE_KEY_ID_USER,
	          		     DA_Private_Key_Size,
				         DA_Private_Key,
				         0);
