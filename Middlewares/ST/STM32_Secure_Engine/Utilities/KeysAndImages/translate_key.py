...
#PUSH {R4, R11}
def build_mov(reg, val, version, reverse=False):
    if version == "V7M":
        if reverse == False:
            ...
            #build the 2 16bits to write
            out ="\tMOVW R"+str(reg)+", #"+hex(val[2]*256+val[3])+"\n"
            out +="\tMOVT R"+str(reg)+", #"+hex(val[0]*256+val[1])+"\n"
    elif version == "V6M":
        if reverse == False:
...
def translate(key, end=False, assembly="IAR", version="V7M", reverse=False):
    if version == "V7M":        
        STMR0 = "\tSTM R0"
        ADDR0 = "\tADD R0"
    elif version == "V6M":    
        ...
        for j in range(0,loop):
            if j!=0:
                ...
                if version == "V7M":
                    output+= ADDR0 +", R0,#16\n"
            for i in range(0,endreg):
                output+=build_mov(4-i, key[16*j+i*4:16*j+i*4+4], version, reverse)
    output += STMR0 + ", {R1-R"+str(endreg)+"}\n"
    output += "\tPOP {R1-R5}\n"
    output += "\tBX LR\n"
    if end:
