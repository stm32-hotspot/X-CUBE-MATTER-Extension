...
#if defined (CKS_ENABLED) || defined (WBSTACK_UPDATE_ENABLED)
#include "se_cm0.h"
#endif /* CKS_ENABLED || (WBSTACK_UPDATE_ENABLED)*/
...
#if defined(SFU_ISOLATE_SE_WITH_FIREWALL) || defined(CKS_ENABLED) ||defined(WBSTACK_UPDATE_ENABLED)
/* Variable used to store Appli vector position */
uint32_t AppliVectorsAddr;
#endif /* (SFU_ISOLATE_SE_WITH_FIREWALL) || (CKS_ENABLED) || (WBSTACK_UPDATE_ENABLED)*/
#if defined(CKS_ENABLED) || defined(WBSTACK_UPDATE_ENABLED)
...
SE_ErrorStatus SE_CallGateService(SE_FunctionIDTypeDef eID, SE_StatusTypeDef *const peSE_Status, va_list arguments)
{
  ...
  switch (eID)
  {
    ...
    case SE_INIT_ID:
    {
      ...
      CM0_Init();
#endif /* defined(CKS_ENABLED) || defined(WBSTACK_UPDATE_ENABLED)*/
...
#if defined (WBSTACK_UPDATE_ENABLED)
    case SE_CM0_UPDATE:
    {
    ...
    case SE_CM0_GET_FWINFO:
    {
      ...
      /* Retrieve argument(s) */
      pWirelessInfo = va_arg(arguments, WirelessFwInfo_t *);
      
      /* Check the destination buffer */
      if (SE_LL_Buffer_in_SBSFU_ram(pWirelessInfo, sizeof(pWirelessInfo)) != SE_SUCCESS)
      {
        e_ret_status = SE_ERROR;
        break;
      }
