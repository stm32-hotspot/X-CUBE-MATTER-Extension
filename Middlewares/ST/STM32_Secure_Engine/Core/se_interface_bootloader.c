...
SE_ErrorStatus SE_CM0_GetFWInfo(SE_StatusTypeDef *pSE_Status, WirelessFwInfo_t *pInfo)
{
  SE_ErrorStatus e_ret_status;
  uint32_t primask_bit; /*!< interruption mask saved when disabling ITs then restore when re-enabling ITs */
  ...
  if (0U != SE_IsUnprivileged())
  {
    SE_APP_ActiveFwInfo_t WirelessInfo;
    uint32_t params[1] = {(uint32_t)pInfo};
    SE_SysCall(&e_ret_status, SE_CM0_GET_FWINFO, pSE_Status, &params);
  }
  else
  {
#endif /* SFU_ISOLATE_SE_WITH_MPU */
    ...
    SE_EnterSecureMode(&primask_bit);
    e_ret_status = (*SE_CallGatePtr)(SE_CM0_GET_FWINFO, pSE_Status, primask_bit, pInfo);
    SE_ExitSecureMode(primask_bit);
#ifdef SFU_ISOLATE_SE_WITH_MPU
  }
#endif /* SFU_ISOLATE_SE_WITH_MPU */
