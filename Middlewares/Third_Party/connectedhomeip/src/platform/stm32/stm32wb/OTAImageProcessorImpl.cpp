...
bool OTAImageProcessorImpl::IsFirstImageRun() {
    ...
    return requestor->GetCurrentUpdateState()
            == OTARequestorInterface::OTAUpdateStateEnum::kApplying;
}
...
void OTAImageProcessorImpl::HandleProcessBlock(intptr_t context) {
    ...
    if (mSwitchDwlSlot == false) { //CPU1 write in DWL_SLOT 1
        if (imageProcessor->mParams.downloadedBytes == 0) { // get STM_Header
            uint8_t STMHeader[STM_HEADER_SIZE];

            memcpy(STMHeader, reinterpret_cast<std::uint8_t*>(imageProcessor->mBlock.data()),
                    sizeof(STMHeader));
            ...
            // check the header
            if ((mCPU1Size > SLOT_SIZE(SLOT_DWL_1)) || (mCPU2Size > SLOT_SIZE(SLOT_DWL_4))
                    || (mCPU2Size + mCPU1Size == 0)) {
                ChipLogError(SoftwareUpdate, "Flash decode failed");
                imageProcessor->mDownloader->EndDownload(CHIP_ERROR_DECODE_FAILED);
                return;
            }

            if (mCPU1Size == 0) { //update only CPU2
                //write in DWL_SLOT1 data without STM header
                status =
                        STM_EXT_FLASH_WriteChunk(
                                imageProcessor->mParams.downloadedBytes + SLOT_DWL_4_START,
                ...
                imageProcessor->mParams.downloadedBytes +=
                ...
                status =
                        STM_EXT_FLASH_WriteChunk(
                                imageProcessor->mParams.downloadedBytes + SLOT_DWL_1_START,
                                reinterpret_cast<std::uint8_t*>(imageProcessor->mBlock.data())
                                        + STM_HEADER_SIZE,
                                static_cast<std::uint32_t>(imageProcessor->mBlock.size())
                                        - STM_HEADER_SIZE);
        ...
        } else if ((imageProcessor->mParams.downloadedBytes
                + static_cast<std::uint32_t>(imageProcessor->mBlock.size())) >= mCPU1Size) {

            //split the block in 2 for Dwl_slot1 and Dwl_slot4
            uint32_t CPU2BlockStart = imageProcessor->mParams.downloadedBytes
                    + static_cast<std::uint32_t>(imageProcessor->mBlock.size()) - mCPU1Size;
            uint32_t CPU1BlockEnd = static_cast<std::uint32_t>(imageProcessor->mBlock.size())
                    - CPU2BlockStart;

            if (CPU2BlockStart + CPU1BlockEnd
                    == static_cast<std::uint32_t>(imageProcessor->mBlock.size())) {
                mDownloadedBytesCPU2 = 0;
